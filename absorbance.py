...
def abs_ParamSearch(x, y, z, data, delta):
    ...
    for v in zip(xv[bl], yv[bl], zv[bl]):
        loglike = -opt_func(v, data, delta)
        #print(' i', i, 'like', loglike, 'a1', xv[bl][i], 'a2', yv[bl][i], 'a3', zv[bl][i], 'like', np.exp(loglike))
        if loglike > best:
...
def main():
    ...
    delta = 1.0                                 # data error size as described in Jackson et al. (2018) > 25, 25, 25 for more accuracy
    na1, na2, na3 = 5, 5, 5                     # Set discretization levels for grid search (only used if optmeth='Grid')
